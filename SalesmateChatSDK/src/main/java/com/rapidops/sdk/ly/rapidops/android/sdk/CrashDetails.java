...
class CrashDetails {
    ...
    private static final int startTime = Rapidops.currentTimestamp();
    ...
    static void addLog(String record) {
        ...
        if(recordLength > maxBreadcrumbSize){
            if(Rapidops.sharedInstance().isLoggingEnabled()){
                Log.d(Rapidops.TAG, "Breadcrumb exceeds character limit: [" + recordLength + "], reducing it to: [" + maxBreadcrumbSize + "]");
            }
        ...
        if(logs.size() > maxBreadcrumbLimit){
            if(Rapidops.sharedInstance().isLoggingEnabled()){
                Log.d(Rapidops.TAG, "Breadcrumb amount limit exceeded, deleting the oldest one");
            }
            logs.removeFirst();
        }
    }
    ...
    static String getBatteryLevel(Context context) {
        try {
            ...
            if (Rapidops.sharedInstance().isLoggingEnabled()) {
                Log.i(Rapidops.TAG, "Can't get batter level");
            }
        }
    ...
    static String getRunningTime() {
        return Integer.toString(Rapidops.currentTimestamp() - startTime);
    }
    ...
    static String isOnline(Context context) {
        try {
            ...
            if (Rapidops.sharedInstance().isLoggingEnabled()) {
                Log.w(Rapidops.TAG, "Got exception determining connectivity", e);
            }
        }
        return null;
    }
