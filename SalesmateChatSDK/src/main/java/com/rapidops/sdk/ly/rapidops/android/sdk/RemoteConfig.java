...
public class RemoteConfig {
    ...
    protected static void updateRemoteConfigValues(final Context context, final String[] keysOnly, final String[] keysExcept, final ConnectionQueue connectionQueue_, final boolean requestShouldBeDelayed, final RemoteConfigCallback callback){
        if (Rapidops.sharedInstance().isLoggingEnabled()) {
            Log.d(Rapidops.TAG, "Updating remote config values, requestShouldBeDelayed:[" + requestShouldBeDelayed + "]");
        }
        ...
        if (Rapidops.sharedInstance().isLoggingEnabled()) {
            Log.d(Rapidops.TAG, "RemoteConfig requestData:[" + requestData + "]");
        }
        ...
        try {
            ...
            if (Rapidops.sharedInstance().isLoggingEnabled()) {
                Log.e(Rapidops.TAG, "IOException while preparing remote config update request :[" + e.toString() + "]");
            }
        ...
        (new RapidopsStarRating.ImmediateRequestMaker()).execute(urlConnection, requestShouldBeDelayed, new RapidopsStarRating.InternalFeedbackRatingCallback() {
            @Override
            public void callback(JSONObject checkResponse) {
                if (Rapidops.sharedInstance().isLoggingEnabled()) {
                    Log.d(Rapidops.TAG, "Processing remote config received response, receved response is null:[" + (checkResponse == null) + "]");
                }
    ...
    protected static class RemoteConfigValueStore {
        ...
        public void mergeValues(JSONObject newValues){
            ...
            while (iter.hasNext()) {
                ...
                try {
                    ...
                    if (Rapidops.sharedInstance().isLoggingEnabled()) {
                        Log.e(Rapidops.TAG, "Failed merging new remote config values");
                    }
                }
            }
        }
        ...
        public static RemoteConfigValueStore dataFromString(String storageString){
            ...
            try {
                ...
                if (Rapidops.sharedInstance().isLoggingEnabled()) {
                    Log.e(Rapidops.TAG, "Couldn't decode RemoteConfigValueStore successfully: " + e.toString());
                }
