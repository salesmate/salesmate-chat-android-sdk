...
public class AdvertisingIdAdapter {
    ...
    public static void setAdvertisingId(final Context context, final mStore store, final DeviceId deviceId) {
            ...
            public void run() {
                try {
                    ...
                    if (t.getCause() != null && t.getCause().getClass().toString().contains("GooglePlayServicesAvailabilityException")) {
                        // recoverable, let device ID be null, which will result in storing all requests to Rapidops Analytics server
     ...
     */
    protected static void cacheAdvertisingID(final Context context, final mStore store) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
