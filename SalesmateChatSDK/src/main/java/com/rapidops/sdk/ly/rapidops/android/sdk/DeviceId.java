...
public class DeviceId {
    /**
     * Enum used throughout Rapidops Analytics which controls what kind of ID Rapidops Analytics should use.
     */
    public enum Type {
        DEVELOPER_SUPPLIED,
        OPEN_UDID,
     ...
     */
    protected DeviceId(mStore store, Type type) {
        if (type == null) {
     ...
     */
    protected DeviceId(mStore store, String developerSuppliedId) {
    ...
    private void retrieveId (mStore store) {
     ...
     * In some cases, Rapidops Analytics can override ID generation strategy to other one, for example when
     * Google Play Services are not available and user chose Advertising ID strategy, it will fall
     * back to OpenUDID
     * @param context Context to use
     * @param store Rapidops Analytics to store configuration in
     * @param raiseExceptions whether to raise exceptions in case of illegal state or not
     */
    protected void init(Context context, mStore store, boolean raiseExceptions) {
        ...
        switch (type) {
            ...
            case ADVERTISING_ID:
                if (AdvertisingIdAdapter.isAdvertisingIdAvailable()) {
                    ...
                    // just do nothing, without Advertising ID and OpenUDID this user is lost for Rapidops Analytics
                    if (Rapidops.sharedInstance().isLoggingEnabled()) {
    ...
    private void storeOverriddenType(mStore store, Type type) {
    ...
    private Type retrieveOverriddenType(mStore store) {
        return retrieveType(store, PREFERENCE_KEY_ID_TYPE);
    }

    private Type retrieveType(mStore store, String preferenceName) {
        // Using strings is safer when it comes to extending Enum values list
    ...
    protected void switchToIdType(Type type, Context context, mStore store) {
        if (Rapidops.sharedInstance().isLoggingEnabled()) {
    ...
    protected String changeToDeveloperProvidedId(mStore store, String newId) {
    ...
    protected void changeToId (Context context, mStore store, Type type, String deviceId) {
        this.id = deviceId;
        this.type = type;
    ...
    protected String revertFromDeveloperId(mStore store) {
    ...
    /**
     * Helper method for null safe comparison of current device ID and the one supplied to Rapidops Analytics.init
