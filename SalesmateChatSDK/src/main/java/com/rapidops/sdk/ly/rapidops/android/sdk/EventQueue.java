...
public class EventQueue {
    private final mStore mStore_;

    /**
     * Constructs an EventQueue.
     * @param mStore backing store to be used for local event queue persistence
     */
    EventQueue(final mStore mStore) {
        mStore_ = mStore;
    }
    ...
    String events() {
        ...
        final List<Event> events = mStore_.eventsList();
    ...
    void recordEvent(final String key, final Map<String, String> segmentation, final Map<String, Integer> segmentationInt, final Map<String, Double> segmentationDouble, final int count, final double sum, final double dur) {
        final long timestamp = Rapidops.currentTimestampMs();
        final int hour = Rapidops.currentHour();
        final int dow = Rapidops.currentDayOfWeek();
        mStore_.addEvent(key, segmentation, segmentationInt, segmentationDouble, timestamp, hour, dow, count, sum, dur);
    }

    void recordEvent(final Event event) {
        event.hour = Rapidops.currentHour();
        event.dow = Rapidops.currentDayOfWeek();
        mStore_.addEvent(event);
    }
