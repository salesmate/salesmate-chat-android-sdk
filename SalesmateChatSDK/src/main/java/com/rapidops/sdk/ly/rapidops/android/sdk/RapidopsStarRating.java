...
public class RapidopsStarRating {
     ...
     * @param callback
     */
    public static void showStarRating(Context context, final RatingCallback callback){
    ...
    public static void showStarRatingCustom(
        ...
        if(!(context instanceof Activity)) {
            if (Rapidops.sharedInstance().isLoggingEnabled()) {
                Log.e(Rapidops.TAG, "Can't show star rating dialog, the provided context is not based off a activity");
            }
            return;
        }
        ...
        ratingBar.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            ...
            public void onRatingChanged(RatingBar ratingBar, float v, boolean b) {
                ...
                if(Rapidops.sharedInstance().getConsent(Rapidops.RapidopsFeatureNames.starRating)) {
                    Map<String, String> segm = new HashMap<>();
                    segm.put("platform", "android");
                    segm.put("app_version", DeviceInfo.getAppVersion(context));
                    segm.put("rating", "" + rating);

                    Rapidops.sharedInstance().recordEvent(STAR_RATING_EVENT_KEY, segm, 1);
                }
    ...
    static class StarRatingPreferences {
        ...
        JSONObject toJSON() {
            ...
            try {
                ...
                if (Rapidops.sharedInstance().isLoggingEnabled()) {
                    Log.w(Rapidops.TAG, "Got exception converting an StarRatingPreferences to JSON", e);
                }
            }
        ...
        static StarRatingPreferences fromJSON(final JSONObject json) {
            ...
            if(json != null) {
                try {
                    ...
                    if (Rapidops.sharedInstance().isLoggingEnabled()) {
                        Log.w(Rapidops.TAG, "Got exception converting JSON to a StarRatingPreferences", e);
                    }
                }
            }
    ...
    /// Rapidops Analytics webDialog user rating
    ...
    protected static synchronized void showFeedbackPopup(final String widgetId, final String closeButtonText, final Activity activity, final Rapidops rapidops, final ConnectionQueue connectionQueue_, final FeedbackRatingCallback devCallback){
        if (Rapidops.sharedInstance().isLoggingEnabled()) {
            Log.d(Rapidops.TAG, "Showing Feedback popup for widget id: [" + widgetId + "]");
        }

        if(widgetId == null || widgetId.isEmpty()){
            if(devCallback != null){
                devCallback.callback("Rapidops Analytics widgetId cannot be null or empty");
            }
            throw new IllegalArgumentException("Rapidops Analytics widgetId cannot be null or empty");
        }

        if(rapidops.getConsent(Rapidops.RapidopsFeatureNames.starRating)) {
            //check the device type
            final boolean deviceIsPhone;
            final boolean deviceIsTablet;
            final boolean deviceIsTv;

            deviceIsTv = RapidopsStarRating.isDeviceTv(activity);

            if(!deviceIsTv) {
                deviceIsPhone = !RapidopsStarRating.isDeviceTablet(activity);
                deviceIsTablet = RapidopsStarRating.isDeviceTablet(activity);
            } else {
            ...
            try {
                ...
                if (Rapidops.sharedInstance().isLoggingEnabled()) {
                    Log.e(Rapidops.TAG, "IOException while checking for rating widget availability :[" + e.toString() + "]");
                }
            ...
            if (Rapidops.sharedInstance().isLoggingEnabled()) {
                Log.d(Rapidops.TAG, "rating widget url :[" + ratingWidgetUrl + "]");
            }
            ...
            (new ImmediateRequestMaker()).execute(urlConnection, false, new InternalFeedbackRatingCallback() {
                ...
                public void callback(JSONObject checkResponse) {
                    if(checkResponse == null){
                        if (Rapidops.sharedInstance().isLoggingEnabled()) {
                            Log.d(Rapidops.TAG, "Not possible to show Feedback popup for widget id: [" + widgetId + "], probably a lack of connection to the server");
                        }
                        if(devCallback != null){
    ...
    protected static class ImmediateRequestMaker extends AsyncTask<Object, Void, JSONObject> {
        ...
        protected JSONObject doInBackground(Object... params) {
            if (Rapidops.sharedInstance().isLoggingEnabled()) {
                Log.v(Rapidops.TAG, "Starting ImmediateRequestMaker request");
            }
            ...
            try {
                if(requestShouldBeDelayed){
                    ...
                    if (Rapidops.sharedInstance().isLoggingEnabled()) {
                        Log.v(Rapidops.TAG, "ImmediateRequestMaker request should be delayed, waiting for 10.5 seconds");
                    }
                    ...
                    try {
                        ...
                        if (Rapidops.sharedInstance().isLoggingEnabled()) {
                            Log.w(Rapidops.TAG, "While waiting for 10 seconds in ImmediateRequestMaker, sleep was interrupted");
                        }
                    }
                }
                ...
                if(wasSuccess) {
                    ...
                    if (Rapidops.sharedInstance().isLoggingEnabled()) {
                        Log.e(Rapidops.TAG, "Encountered problem while making a immediate server request, :[" + buffer.toString() + "]");
                    }
                    return null;
                }
